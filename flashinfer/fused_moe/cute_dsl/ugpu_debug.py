...
"""One-shot debug tracing for the uGPU localized MoE path.
...
Opt-in via FLASHINFER_UGPU_DEBUG=1, and each tag prints once per process, so
...
def ugpu_debug_enabled() -> bool:
    return os.environ.get("FLASHINFER_UGPU_DEBUG", "0") == "1"


def ugpu_trace(tag: str, msg: str) -> None:
    """Print `msg` once per process for this `tag`. No-op unless enabled."""
    if not ugpu_debug_enabled() or tag in _SEEN:
        return
    _SEEN.add(tag)
    print(f"[ugpu-fi] {msg}", file=sys.stderr, flush=True)
