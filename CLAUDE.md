...
| Trace the MoE path localized across locality domains | `export FLASHINFER_LOCALIZED_MOE_DEBUG=1` |
...
| `FLASHINFER_LOCALIZED_MOE_DEBUG` | `0` | `flashinfer/fused_moe/cute_dsl/localized_moe_debug.py` | `1` prints a one-shot trace of the MoE path localized across locality domains to stderr: the `_moe_core_impl` fan-out, and each dispatcher's `partition_id`/`domain_id`, column offsets, and `sm_count`/`max_active_clusters` grid sizing. Everything a caller can observe about localization happens before the FlashInfer boundary, and the grid sizing has no correctness signature, so it cannot be checked from outside. One line per tag per process. |
